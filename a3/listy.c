...
{
    // TODO: Complete the function.
    if(list == NULL || strcmp(new->val->start_date, list->val->start_date)<=0){
    ...
    while(cur != NULL) {
        if (strcmp(new->val->start_date, prev->val->start_date)>=0 && strcmp(new->val->start_date, cur->val->start_date)<=0) {
            break;
        }
        prev = cur;
        cur = cur->next;    
